...
def add_mask2_kernel(x_ptr, z_ptr, N0, B0: tl.constexpr):
    ...
    pid = tl.program_id(axis=0)
    block_start = pid * B0
    offset = tl.arange(0, B0) + block_start
    mask = offset < N0
...
def add_vec_kernel(x_ptr, y_ptr, z_ptr, N0, N1, B0: tl.constexpr, B1: tl.constexpr):
    ...
    y = tl.load(y_ptr + off_y, mask=mask_y, other=0)

    z = x[None, :] + y[:, None]      #广播

    z_offset = off_y[:, None] * N0 + off_x[None, :]
    mask = mask_x[None, :] & mask_y[:, None]
    tl.store(z_ptr + z_offset, z, mask=mask)
    return
...
def mul_relu_block_back_kernel(
    ...
    # 加载 x, y, dz
    x = tl.load(x_ptr + off_y[:, None] * N0 + off_x[None, :], mask=mask_y[:, None] & mask_x[None, :])
    y = tl.load(y_ptr + off_y, mask=mask_y)
    dz = tl.load(dz_ptr + off_y[:, None] * N0 + off_x[None, :], mask=mask_y[:, None] & mask_x[None, :])
    ...
    tl.store(dx_ptr + off_y[:, None] * N0 + off_x[None, :], dx, mask=mask_y[:, None] & mask_x[None, :])
...
def sum_kernel(x_ptr, z_ptr, N0, N1, T, B0: tl.constexpr, B1: tl.constexpr):
    ...
    for off in range(0, T, B1):
        offs = tl.arange(0, B1) + off
        mask = offs < T
        x = tl.load(x_ptr + off_i * T + offs, mask=mask, other=0.0)
        acc += tl.sum(x, axis=0)
    tl.store(z_ptr + off_i, acc)
    return
...
def softmax_kernel(x_ptr, z_ptr, N0, N1, T, B0: tl.constexpr, B1: tl.constexpr):
    ...
    for off in range(0,T,B1):
        off_j = tl.arange(0,B1)+ off
        offij = off_i[:, None]*T + off_j[None, :]
        mask_j = off_j < T
        mask = mask_i[:, None] & mask_j[None, :]
        x = tl.load(x_ptr + offij, mask=mask, other=0.0)
        new_x_max = tl.maximum(tl.max(x, axis=1), x_max)
        new_exp = tl.exp2(log2_e * (x - new_x_max[:, None]))
        factor = tl.exp2(log2_e * (x_max- new_x_max))
        exp_sum = exp_sum*factor + tl.sum(new_exp, axis=1)
        x_max = new_x_max
    for off in range(0,T,B1):
        off_j = tl.arange(0,B1)+ off
        offij = off_i[:, None]*T + off_j[None, :]
        ...
        tl.store(z_ptr + offij, z, mask=mask)
...
def softmax_kernel_brute_force(
    ...
    for off in range(0, T, B1):
        off_j = tl.arange(0, B1) + off
        off = off_i[:,None] + off_j[None,:]
        mask_j = off_j < T
        mask = mask_i[:None] & mask_j[None, :]
        x = tl.load(x_ptr + off, mask=mask, other=0.0)
        x_max = tl.maximum(tl.max(x, axis=1), x_max)
    for off in range(0, T, B1):
        off_j = tl.arange(0, B1) + off
        off = off_i[:,None] + off_j[None,:]
        mask_j = off_j < T
    ...
    for off in range(0, T, B1):
        ...
        exp_x = tl.exp2(log2_e * (x - x_max[:, None]))
        z = exp_x / exp_sum[:, None]
        tl.store(z_ptr + off, z, mask=mask)
    return
...
def flashatt_kernel(
    ...
    for off in range(0, T, B1):
        off_j = off + tl.arange(0, B1)
        mask_j = off_j < T
        mask = mask_i[:, None] & mask_j[None, :]

        k  =tl.load(k_ptr + off_j, mask=mask_j)
        qk = q[:, None] * k[None, :] + tl.where(mask, 0, -1.0e6)
        new_qk_max = tl.maximum(tl.max(qk, axis=1), qk_max)
        new_exp  = tl.exp2(log2_e * (qk - new_qk_max[:, None]))
        factor = tl.exp2(log2_e * (qk_max - new_qk_max))
        exp_sum  = factor * exp_sum + tl.sum(new_exp, axis=1)
        v = tl.load(v_ptr + off_j, mask=mask_j, other=0.0)
        z = z*factor + tl.sum(new_exp * v[None, :], axis=1)
        qk_max = new_qk_max
...
def conv2d_kernel(
    ...
    k = tl.load(k_ptr + off_hw, mask=off_hw < KH * KW)
    for j in range(0,H):
        for i in range(0,W):
            off_j = j + off_h[None,:, None]
            off_i1 = i + off_w[None,None, :]
            off = off_i * H * W + off_j * W + off_i1
            mask = (off_j< H) & (off_i1 < W) 
            x = tl.load(x_ptr + off, mask=mask, other=0.0)
            z = tl.sum(x* k)
            off_z = off_i* H * W + j * W + i
...
def quant_dot_kernel(
    ...
    for l in tl.range(0, MID, B_MID):
        ...
        off_weight_l = (l // GROUP) + tl.arange(0, B_MID // FPINT)
